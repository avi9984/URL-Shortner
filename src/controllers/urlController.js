...
const redisClient = redis.createClient(
  15331,
  'redis-15331.c305.ap-south-1-1.ec2.cloud.redislabs.com',
  { no_ready_check: true }
);

//Authenticating the client (our application)
redisClient.auth("qrTgjCdwlHFgrITMYSqDZJSufYWfcUye", function (err) {
  if (err) throw err;
});
  ...
  try {
    ...
    if (Object.keys(data).length == 0) {
      return res
        ...
        .send({
          status: false,
          message: "Invalid Url please provide valid details",
        });
    }
    ...
    if (!isValid(data.longUrl)) {
      return res
        .status(400)
        .send({ status: false, message: "Please give the long URL" });
    }
    ...
    let checkUrl = await Url.findOne({ longUrl: data.longUrl }).select({
      _id: 0,
      __v: 0,
      createdAt: 0,
      updatedAt: 0,
    ...
    if (checkUrl) {
      ...
      await SETEX_ASYNC(`${data.longUrl}`, 30, JSON.stringify(checkUrl));
      return res
        .status(200)
    ...
    let responseData = await Url.findOne({ urlCode: urlCode }).select({
      _id: 0,
      __v: 0,
      createdAt: 0,
      updatedAt: 0,
    ...
    await SETEX_ASYNC(`${data.longUrl}`, 30, JSON.stringify(responseData));
    res
      .status(201)
      .send({
        status: true,
